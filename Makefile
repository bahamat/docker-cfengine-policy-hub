...
IMAGEBASE=cfengine
PLATFORM=debian
IMAGENAME=$(OWNER)/$(IMAGEBASE)-$(PLATFORM)
...
# inside the container, TEST_LOGDIR takes the values CONTAINER_LOGDIR because it's remapped
	TEST_LOGDIR := $(WITH_TESTFILES)/testruns/$(PLATFORM)/$(TEST_DATE)
	CONTAINER_LOGDIR := $(CONTAINER_TESTFILES)/testruns/$(PLATFORM)/$(TEST_DATE)
endif
...
build:
	VERSION=$(VERSION) $(DOCKER) build $(BUILDFLAGS) -t="$(IMAGENAME):$(VERSION)" -f Dockerfile.$(PLATFORM) .
